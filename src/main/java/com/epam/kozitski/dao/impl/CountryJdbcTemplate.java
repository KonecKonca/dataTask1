...
public class CountryJdbcTemplate implements CountryDao {
    ...
    // HERE for test i changed mapper on default
    @Override
    public Optional<Country> searchById(long id) {
        return Optional.ofNullable(jdbcTemplate.getJdbcOperations()
                .queryForObject(SEARCH_BY_ID_SQL, new BeanPropertyRowMapper<>(Country.class), id));
    }
    @Override
